<?php

session_start();

?>

<?php

include('database.php'); 

$id = $_GET['id'];
$sql = "SELECT * FROM device WHERE id = {$id}";
$query = mysqli_query($connection, $sql); //ส่งsqlไปrunในdatabase
$device = mysqli_fetch_assoc($query);

$html = '';

while ($row = mysqli_fetch_assoc($query)) {
  $update = "
      <a href='device_update_forms.php?id={$row['id']}' class='cancel cancelbtn btn button1' style='color: #C70039;'>
          UPDATE
      </a>
  ";

  $delete ="

      <a href = 'device_delete_forms.php?id={$row['id']}' class='cancel cancelbtn btn button1' style='color: #C70039;'>
          DELETE
      </a>
  ";

  $report ="
      <a href = 'user_report_forms.php?id={$row['id']}' class='cancel cancelbtn btn button1' style='color: #C70039;'>
        REPORT
      </a>
  ";

  $image = '<img src="' . 'http://localhost/borrow/upload/' . basename($row['image']) . '" width="50" height="50">';

  if ($row['image'] == '') {
     $image = '';
  }
}

?>


<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.0-beta2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-BmbxuPwQa2lc/FVzBcNJ7UAyJxM6wuqIj61tLrc4wSX0szH/Ev+nYRRuWlolflfl" crossorigin="anonymous">
    <title>Borrow Device</title>
    <!-- <link rel="stylesheet" href="//code.jquery.com/ui/1.12.1/themes/base/jquery-ui.css"> -->
    <!-- <link rel="stylesheet" href="/resources/demos/style.css"> -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
    <!-- <script src="https://code.jquery.com/jquery-1.12.4.js"></script> -->
    <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.5.1/jquery.min.js" integrity="sha512-bLT0Qm9VnAYZDflyKcBaQ2gg0hSYNQrJ8RilYldYQ1FxQYoCLtUjuuRuZo+fjqhx/qtq/1itJ0C2ejDxltZVFg==" crossorigin="anonymous"></script>
    <!-- <script src="https://code.jquery.com/ui/1.12.1/jquery-ui.js"></script> -->
    <script src="moment.min.js"></script>
    <script src="bootstrap-datetimepicker.min.js"></script>
    <script src="tempusdominus-bootstrap-4.js"></script>
    <link rel="stylesheet" href="bootstrap-datetimepicker.min.css">



    <script type="text/javascript">

      $(function () {
        $('#datetimepicker1').datetimepicker({
          format: 'YYYY-MM-DD HH:mm:00'
        });

        $('#borrow_date').on('input', function () {
          // $('#show_select_borrow_date').text($('#borrow_date').val());
          borrow_date = $('#borrow_date').val();
          device_id = <?php echo $id; ?>;

          $.ajax({
            url: "ajax_get_device_amount.php?id=" + device_id + "&borrow_date=" + borrow_date, 
            success: function (result) {
              $("#device_amount").html(result);
              $("#hidden_device_amount").val(result);
            }
          });
        });

        $('#datetimepicker2').datetimepicker({
          format: 'YYYY-MM-DD HH:mm:00'
        });

        $('form').submit(function (event) {
          borrow_date = $('input[name="borrow_date"]').val();
          return_date = $('input[name="return_date"]').val();
          validate = true;
          message = 'ข้อมูลที่กรอกไม่ถูกต้อง กรุณาตรวจสอบข้อมูลต่อไปนี้\n';

          if (borrow_date == '') {
            message += '- วันที่ยืม\n';
            validate = false;
          }

          if (return_date == '') {
            message += '- วันที่คืน\n';
            validate = false;
          } 

          check = moment(borrow_date).isBefore(return_date);
          
          if (check == false) {
            message += '- วันที่ยืมต้องน้อยกว่าวันที่คืน\n';
            validate = false;
          }

          amount = parseInt($('input[name="amount"]').val()); //เก็บamountเข้าตัวแปร
          checkamount = Number.isInteger(amount);

// alert(test);
// alert(checkamount);

          if (checkamount == false) {
            message += '- จำนวณยืมผิดพลาด\n';
            validate = false;
          }

          if (amount > <?php echo $device['totalamount'] ?>){
            message += '- อุปกรณ์ไม่พอ\n'  + amount + '>' + <?php echo $device['totalamount'] ?>;
            validate = false;
          }
          //+ amount + ' > ' device_amount
          // + amount + '>' + <?php echo $device['totalamount'] ?>

          if (validate == false) {
            alert(message);
            event.preventDefault();
          }

          device_amount = Number.parseInt($('#device_amount').text());

          if (amount > device_amount) {
            $('#queue').val(1);

            answer = confirm("จำนวณที่ยืมมากกว่าของในระบบ ต้องการเข้าคิวมั้ย");

            if (answer == false){
              event.preventDefault();
            }
          }
        });
      });
    </script>

  </head>

  <style>
    .logout {
      border: none;
      color: white;
      padding: 5px 20px;
      text-align: center;
      text-decoration: none;
      display: inline-block;
      font-size: 16px;
      margin: 4px 2px;
      transition-duration: 0.4s;
      cursor: pointer;
    }

    .center {
      margin: auto;
      width: 60%;

    }

    .cancel {
      border: none;
      color: white;
      padding: 1px 5px;
      text-align: center;
      text-decoration: none;
      display: inline-block;
      font-size: 16px;
      margin: 4px 2px;
      transition-duration: 0.4s;
      cursor: pointer;
    }
    
    .logoutbtn {
      color: black; 
      border: 1px solid #C70039;
    }

    .cancelbtn {
      color: black; 
      border: 1px solid #C70039;
    }
  </style>

<header>
        <nav class="navbar navbar-expand-md navbar-dark bg-dark">
        <div class="container">
           <b class="navbar-brand" style="font-size=100px;">EGAT</b>
            <div class="collapse navbar-collapse" id="navbarCollapse">
            <ul class="navbar-nav me-auto ">
              <li class="nav-item">
                <a class="nav-link active mt-3" aria-current="page" href="devicelist.php" >
                  <h5>
                    Home 
                    <img class="mb-1" style="width: 25px; height: 25px;" src="http://localhost/borrow/upload/house.svg" />
                  </h5>
                </a>
              </li>
<?php
  if($_SESSION['user_role'] == 'user') {

?>
              <li class="nav-item">
                <a class="nav-link active mt-3" aria-current="page" href="circulation_user.php">
                  <h5>
                    Circulation
                    <img style="width: 25px; height: 25px;" src="http://localhost/borrow/upload/operation.svg"></img>
                  </h5>
                </a>
              </li>

<?php

}
// END เมนูแอดมิน

?>

<?php

// BEGIN เมนูแอดมิน
if ($_SESSION['user_role'] == 'admin') {

?>

              <li class="nav-item">
                <a class="nav-link active mt-3" aria-current="page" href="manage_device.php"><h5> Manage Device <img style="width: 30px; height: 30px;" src="http://localhost/borrow/upload/device.svg" /></h5></a>
              </li>
              <li class="nav-item">
                <a class="nav-link active mt-3" aria-current="page" href="manage_user.php"><h5> Manage User <img style="width: 30px; height: 30px;" src="http://localhost/borrow/upload/team.svg" /></h5></a>
              </li>
              <li class="nav-item">
                <a class="nav-link active mt-3" aria-current="page" href="circulation_admin.php">
                  <h5>
                    Circulation
                    <img style="width: 25px; height: 25px;" src="http://localhost/borrow/upload/operation.svg"></img>
                  </h5>
                </a>
              </li>
<?php
}
// END เมนูแอดมิน

?>

            </ul>

            <div class="dropdown text-end">
            <a href="#" class="d-block link-dark text-decoration-none dropdown-toggle" id="dropdownUser1" data-bs-toggle="dropdown" aria-expanded="false">
              <img src="data:image/svg+xml;base64," alt="mdo" width="40" height="40" class="rounded-circle">
            </a>
            <ul class="dropdown-menu text-small" aria-labelledby="dropdownUser1">
              <li><a class="dropdown-item" href="logout_do.php">Sign out</a></li>
            </ul>
          </div>

            </div> 
        </div>
        </nav>
    </header> 

    <body class="bg-light">
        <div class="container">
          <main>
            <div class="row mt-5">
              <div class="col-lg-3">
                  <div class="container">
                    <div class="row">
                      <div>
                        <span class="text-muted"><h4>Device Information</h4></span>
                      </div>

                      <div class="col">
                        <div class="card shadow-sm">

                          <img src="http://localhost/borrow/upload/<?php echo basename($device["image"]); ?>" class="card-img-top" width="100%" height="225"  role="img" preserveAspectRatio="xMidYMid slice" focusable="false"><rect width="100%" height="100%" fill="#55595c"></rect><text x="50%" y="50%" fill="#eceeef" dy=".3em"></text></img>

                          <!-- ' . $image . ' -->
                          <div class="card-body">
                            <p class="card-text ">Type: <?php echo $device['type']; ?></p>
                            <p class="card-text ">Brand: <?php echo $device['brand']; ?></p>
                            <p class="card-text ">Model: <?php echo $device['model']; ?></p>
                            <p class="card-text ">Detail: <?php echo $device['detail']; ?></p>
                            <div class="d-flex justify-content-between align-items-center">
                            <small>Amount: 
                              <span id="device_amount">
                                <?php echo $device['amount']; ?>
                              </span>
                            </small>
                            </div>
                          </div>
                        </div>
                      </div>

                    </div>
                  </div>
                </div>
              <div class="col-lg-9 order-md-last">
                <div class="container">
                <div class="row">
                <h4 class="d-flex mb-3">
                  <span class="text-muted ">Device Information</span>
                </h4>

                <form action="borrow_do.php" method="POST">
                  <input type="hidden" name="device_id" value="<?php echo $id; ?>">
                  <input type="hidden" name="device_amount" id="hidden_device_amount">
                  <input type="hidden" name="queue" id="queue">

                  <div class="row">
                    <div class="col-sm-6">
                        <div class="form-group">
                        <label for="address" class="form-label">Borrow Date</label>
                            <div class="input-group date" id="datetimepicker1" data-target-input="nearest">
                                <input type="text" class="form-control datetimepicker-input" data-target="#datetimepicker1" name="borrow_date" id="borrow_date">
                                <div class="input-group-append" data-target="#datetimepicker1" data-toggle="datetimepicker">
                                    <div class="input-group-text" style="width: 40px; height: 40px;"><i class="fa fa-calendar"></i></div>
                                </div>
                            </div>
                        </div>

                        <div id="show_select_borrow_date"></div>
                    </div>

                  </div>

                  <div class="row">
                  <div class="col-sm-6">
                      <div class="form-group">
                      <label for="address" class="form-label">Return Date</label>
                          <div class="input-group date" id="datetimepicker2" data-target-input="nearest">
                              <input type="text" class="form-control datetimepicker-input" data-target="#datetimepicker2" name="return_date">
                              <div class="input-group-append" data-target="#datetimepicker2" data-toggle="datetimepicker">
                                  <div class="input-group-text" style="width: 40px; height: 40px;"><i class="fa fa-calendar"></i></div>
                              </div>
                          </div>
                      </div>
                  </div>

                </div>

                    <div class="col-12">
                      <label for="exampleFormControlTextarea1" class="mb-1">Amount</label>
                      <input type="text" class="form-control" id="exampleFormControlTextarea1" name="amount"></input>

                    </div>

                    <div class="form-group mt-1">
                      <label for="exampleFormControlTextarea1" class="mb-1">Note</label>
                      <textarea type="input" class="form-control" id="exampleFormControlTextarea1" rows="3" name="note"></textarea>
                    </div>

                    <br>

                    <!-- <div class="form-group mt-1 " >
                    <strong for="exampleFormControlTextarea1" class="mb-1">Please press the queue button when making a reservation</strong>

                    <button class='cancel cancelbtn btn button1' style='color: #C70039;' disabled>Queue</button>

                      <textarea type="input" class="form-control" id="exampleFormControlTextarea1" rows="3" name="note"></textarea>
                    </div> -->

                  </div>
              </div>



              <br>
           
              <div class="row">
                  <div class="col-md-6">
                    <button type="submit" style="background-color: #4CAF50;" class="w-100 btn btn-lg"><div style="color: aliceblue;" >Confirm</div></button>                  
                  </div>
                  <div class="col-md-6">
                    <a href="devicelist.php" style="background-color: #C70039;" class="w-100 btn-lg btn"><div style="color: aliceblue;" >Cancle</div></a>                  
                  </div>
                </div>
  </form>


                  <hr class="my-4">
        
                </div>
               
<!--
            <script src="/docs/5.0/dist/js/bootstrap.bundle.min.js" integrity="sha384-b5kHyXgcpbZJO/tY9Ul7kGkf1S0CWuKcCD38l8YkeH8z8QjE0GmW1gYU5S9FOnJ0" crossorigin="anonymous"></script>
            <script src="form-validation.js"></script>
-->
            <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.0-beta2/dist/js/bootstrap.bundle.min.js" integrity="sha384-b5kHyXgcpbZJO/tY9Ul7kGkf1S0CWuKcCD38l8YkeH8z8QjE0GmW1gYU5S9FOnJ0" crossorigin="anonymous"></script>

        </body>
</html>