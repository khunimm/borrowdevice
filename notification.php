...
$sql = "
  SELECT
    ...
    u.name AS user_name,
    c.borrow_date,
    c.system_return_date AS return_date,
...
while ($row = mysqli_fetch_assoc($query)) {
  ...
  if ($row['admin_reason'] != NULL) {
    $message = 'การยืมอุปกรณ์ของคุณถูกปฏิเสธ';
  } 
...
while ($row = mysqli_fetch_assoc($query)) {
  ...
  $datereturn = '';
  $datereturn .= strtotime("{$row['return_date']}");
  $datetest = '';
  $datetest = strtotime(date("Y-m-d H:i:00"));

  if($datetest >= $datereturn){
    $message  = 'เลยกำหนดการคืนของ';
  }
