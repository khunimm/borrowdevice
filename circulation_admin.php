...
$sql = "
  ...
  WHERE c.user_id_admin_allow IS NULL 
  AND c.admin_reason = ''
";
...
while ($row = mysqli_fetch_assoc($query)) {
  ...
  //   $user = $row['user_code'] . ' ' . $row['user_name'];
  // }

  $allow = "
    <a href='circulation_admin_allow_do.php?id={$row['id']}' class='w3-button w3-teal mt-3'>Allow</a>
  ";

  $cancel = "
    <a href='circulation_admin_cancel_form.php?id={$row['id']}&device_id={$row['device_id']}' class='w3-button w3-red mt-3'>Cancel</a>
  ";
...
$sql = "
  ...
  WHERE c.user_return_date IS NOT NULL
...
while ($row = mysqli_fetch_assoc($query)) {
  ...
  $receive = "
    <a href='circulation_admin_receive_do.php?id={$row['id']}' class='w3-button w3-teal mt-3'>Receive</a>
  ";

  $admin_return = get_user_string($row['user_id_admin_return']);
  $userborrow = get_user_borrow_string($row['user_id_borrow']);

  if ($admin_return == NULL) {
    $admin_return = '[ รอข้อมูล ]';
  }

  if($row['user_id_admin_return'] != NULL){
    $receive = "[received]";
  }
...
$sql = "
  ...
  LEFT JOIN user u
  ON u.id = c.user_id_admin_allow
  WHERE c.user_id_admin_allow IS NOT NULL
  AND c.user_id_admin_return IS NULL
  AND c.admin_reason = ''
  ORDER BY c.user_return_date DESC
";
$query = mysqli_query($connection, $sql); //ส่งsqlไปrunในdatabase
...
while ($row = mysqli_fetch_assoc($query)) {
  ...
  if($datetest >= $datereturn){
    ...
    <tr>
      ...
      <td style='color:red'>{$userborrow}</td>
      <td style='color:red'>
        Code: {$row['device_code']}<br>
        Type: {$row['device_type']}<br>
        Brand: {$row['device_brand']}
      ...
      <td style='color:red'>{$row['amountc']}</td>
      <td style='color:red'>{$note}</td>
      <td style='color:red'>{$user}</td>
      <td style='color:red'> user ยืมอุปกรณ์เลยเวลาที่กำหนด </td>
      ";
  }
  else{
  $html_used_device .= "
...
if ($_SESSION['user_role'] == 'user'){
    ?>
              <li class="nav-item">
                <a class="nav-link active mt-3" aria-current="page" href="circulation.php">
                  <h5>
                    Circulation
...
?>
    ...
    <div class="container">
    <h4>Circulation Admin </h4>
    <body>
    <tbody>
      <button id="buttonallow">test</button>
      <div class="table-responsive" id="allowusertoborrowthedevice">
        <div class="my-3 p-3 bg-body rounded shadow-sm">
          <div class="row">
            <div class="col-md-5 ">
              <h4 class="center">Allow User To Borrow The Device</h4>
            </div>
          </div>
    ...
    <script>
      const targetDiv = document.getElementById("allowusertoborrowthedevice");
      const btn = document.getElementById("buttonallow");
  ...
  <div class="my-3 p-3 bg-body rounded shadow-sm">
        <div class="row">
          <div class="col-md-5 ">
          <h4 class="center">Device is being used</h4>
          </div>
        <br>
...
<br>
    ...
    <div class="my-3 p-3 bg-body rounded shadow-sm">
        <div class="row">
          <div class="col-md-5 ">
          <h4 class="center">Confirm The Return Of The Device</h4>
          </div>
        <br>
